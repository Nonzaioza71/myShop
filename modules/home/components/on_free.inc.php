<div class="flex" style="align-items: center;">
    ...
    <div class="track-wrapper w-auto">
        <ul class="track grid grid-cols-5 overflow-hidden" id="track_on_free">
            ...
            <?php foreach ($products as $idx => $item) {
                if ((@$item['product_promotion_name'] == 'On Free')) { ?>
                    <li id="track_on_free_item" onclick="window.location.href = '?app=home&v=detail&idx=<?= $item['id'] ?>'" class="flex flex-shrink-0 relative w-25 md:w-full sm:w-auto shadow track__item hover:shadow-2xl hover:shadow-green-500 cursor-pointer transform scale-100 hover:scale-125 z-0 hover:z-40 duration-200">
    ...
    <div class="start-5 absolute align-middle m-auto ps-2">
        <button onclick="new OnFree()._goPrev()" class="rounded-full border border-5 p-1 border-opacity-100 bg-white hover:text-gray-500 hover:bg-gray-950 duration-300">
    ...
    <div class="absolute align-middle m-auto ps-2 right-24">
        <button onclick="new OnFree()._goNext()" class="rounded-full border border-5 p-1 border-opacity-100 bg-white hover:text-gray-500 hover:bg-gray-950 duration-300">
            <svg class="dark:text-gray-900" width="64" height="64" viewBox="0 0 8 14" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<div class="w-full">
    <h1 class="text-3xl">
        <?php $type = $_GET['type']; $_count = array_filter($products, function ($item) use ($type) { return $item['product_type_name'] == $type; }); ?>
...
<script>
    class OnFree {
        init() {
            const element = document.querySelector('#track_on_free')
        ...
        _goNext() {
            const element = document.querySelector('#track_on_free')
            const step = document.querySelector('#track_on_free_item').clientWidth * 1
            element.scrollTo({
